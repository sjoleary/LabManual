...
Complete lab 3 ([tidyverse tutorial](https://labmanual.netlify.app/welcome-to-the-tidyverse.html)) and upload the knitted `html` document through Canvas (`HW-2`)^[Remember, in order to knit a document all the code chunks must be able to run properly - so make sure that you delete any "experimental code chunks" (e.g. if you tried something that didn't work).].
